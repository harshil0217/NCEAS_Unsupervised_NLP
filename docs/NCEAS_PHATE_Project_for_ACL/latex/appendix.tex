...
Ensure that:
- All subtopics can increase and decrease in magnitude, but do not contain increase or decrease or any causal relations
- Each subtopic is distinct and separate from others.
\end{lstlisting}
\end{tcolorbox}

\subsubsection*{Generated output}
The model used for recursive generation of hierarchies was \texttt{gpt-4o} with the default temperature of $1.0$ applied across all datasets. We inserted synonyms as would be expected in real data. For each concept in the hierarchy, we asked GPT for synonyms and inserted them into the hierarchy without children, alongside the original concept. For the simpler task of synonym generation we used \texttt{gpt-4o-mini} to improve efficiency. After the dataset was created, we also injected noise by sampling concepts uniformly at random from across the hierarchy and asking \texttt{gpt-4o} to generate concepts that represented the overlap of all the words in the sample.
