...
\begin{description}
    ...
    \item[PaCMAP] (Pairwise Controlled Manifold Approximation) balances preservation of both local and global structure by explicitly controlling pairwise relationships during embedding optimization.
\end{description}
...
In addition to ARI, we also report standard clustering metrics...

We also analyze global structure preservation using Shepard diagrams, which compare pairwise distances in high- and low-dimensional spaces.
...
These evaluation choices are motivated by prior work...
...
Clustering is performed on each reduced embedding using Agglomerative Clustering, HDBSCAN, and Diffusion Condensation.
\subsection{Datasets}
...
To further evaluate generalization to real-world textual data, we additionally conducted experiments on the arXiv and RCV1 datasets. 
