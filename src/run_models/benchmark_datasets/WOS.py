...
shuffle_idx = np.random.RandomState(seed=67).permutation(len(wos))
topic_data = wos.iloc[shuffle_idx].reset_index(drop=True)
...
for embedding_model in embedding_model_names:
    ...
    embedding_list = get_embeddings(wos['topic'], model=embedding_model)

    np.save(f"{embedding_model}_embeddings/wos_embed.npy", embedding_list)

    embedding_list = np.load(f"{embedding_model}_embeddings/wos_embed.npy")
    ...
    np.save(f"{embedding_model}_reduced_embeddings/PHATE_wos_embed.npy", embed_phate)
    ...
    if include_pca:
        ...
        np.save(f"{embedding_model}_reduced_embeddings/PCA_wos_embed.npy", embedding_methods_for_model["PCA"])
    ...
    if include_umap:
        ...
        np.save(f"{embedding_model}_reduced_embeddings/UMAP_wos_embed_new.npy", embedding_methods_for_model["UMAP"])
    ...
    np.save(f"{embedding_model}_reduced_embeddings/tSNE_wos_embed.npy", embedding_methods_for_model["tSNE"])
...
scores_df.to_csv(f"results/wos_clustering_scores.csv", index=False)
