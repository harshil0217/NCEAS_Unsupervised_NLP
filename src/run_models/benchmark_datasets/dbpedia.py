...
shuffle_idx = np.random.RandomState(seed=67).permutation(len(db))
topic_data = db.iloc[shuffle_idx].reset_index(drop=True)
...
scores_df.to_csv(f"results/db_clustering_scores.csv", index=False)
